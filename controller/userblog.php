...
class userblog extends top
{
	...
	{		
		...
		$index=$this->spArgs('page',1);
		$index=$index-1;
    $tweets=$this->getModel("Tweet")->getTweets(10,$this->user_data['id'],$index); 
	 $this->tweets=$tweets['tweets'];
$this->user_data['latestonline']=strtotime($this->user_data['latestonline']);
			$pg = array('uid'=>$this->user_data['id']);
			
$this->pager=$this->getModel('tweet')->pagerHtml($index, 10,'tweet',array('user_id'=>$this->user_data['id']),$c='userblog',$a='index',$pg);
	...
	private function getMyFollow()
	{
     //显示我关注的24个
	    ...
	    $this->follow =$this->getModel('fan')->getFans($this->user_data['id'],1,0,24);
	}
	
	/*谁关注我*/
	private function getMyLook()
	{
		$this->myLook  =$this->getModel('fan')->getFans($this->user_data['id'],0,0,24);
	}
	
	/*我是否关注*/
	private function isFollow()
	{
		if($_SESSION['uid'] == $this->user_data['id'])
		{
			return -1;  //自己
		}
		
		$follow = spCLass('db_fan')->find( array('fan'=>$_SESSION['uid'],'follower'=>$this->user_data['id']));
		if(is_array($follow))
		{
			return 1;  //已关注
		}
	...
	*/
	private function getUserSkin($bid=0)
	{
	 if($this->spArgs('uid') != ''){
		...
		$skin = spClass('db_theme')->find(array('uid'=>$rs['id']));
		$this->user_data = $rs;
...
//		}
		...
		$this->username = $this->user_data['uname'];
