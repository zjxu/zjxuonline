...
class Controller extends JJBase
{
	...
	public function getGetInt($name = '',$default=0) {
		if ($name==''||empty($name)) return $default;
		if (!isset($_GET[$name])||!is_numeric($_GET[$name])) return $default;
	...
	public function getPostInt($name = '',$default=0) {
		if ($name==''||empty($name)) return $default;
		if (!isset($_POST[$name])||!is_numeric($_POST[$name])) return $default;
	...
	public function setCookie($name, $val, $expire = '', $path = '', $domain = '') {
		$expire = (empty($expire)) ? time() + 360024300 : $expire;
