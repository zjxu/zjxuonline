...
function goTwoShow($params)
{

	$id   = $params['id'];     //判断是否存在uid


		return spUrl('two','show',array('id'=>$id));
	
}
spAddViewFunction('goTwoShow','goTwoShow'); 
...
function goUserHome($params)
{
	...
	$uid   = $params['uid'];     //判断是否存在uid
		...
		return spUrl('userblog','index',array('uid'=>$uid));
...
bid domain uid
*/
function goTwoById($params)
{
	$id = $params['id']; //判断是否存在domain
...
function notice_gomess($params)
{
	$uid=$params['uid'];
		return spUrl('user','pm',array('look'=>$uid));
}

spAddViewFunction('notice_gomess','notice_gomess'); 
/*处理通知的链接*/
...
/*处理显示首页或推荐的博文类型feeds*/
function feddshtml($twos,$index=1,$type='index')
{
	$site_uri = trim(dirname($GLOBALS['G_SP']['url']["url_path_base"]),"\/\\");
	if( '' == $site_uri ){ $site_uri = 'http://'.$_SERVER["HTTP_HOST"]; 	}else{ $site_uri = 'http://'.$_SERVER["HTTP_HOST"].'/'.$site_uri; }
	...
	$msg = '';
		...
		foreach($twos as $d)
		{
			...
			if(count($d['images'] )==0)//文字
			{
				$msg .= '<div class="box">
				<div class="textcontents">'.$d['title'].'</div>
				<span class="tag">'.$flag.$d['type'].'</span>
				<a class="boxhover" href="'.goTwoShow(array('id'=>$d['id'])).'" target="_blank">
					<div class="img"><img src="'.avatar(array('uid'=>$d['user_id'],'size'=>'m')).'" alt=""/></div>
				<div class="p">来自:'.$d['uname'].'</div>
				</a>
				</div>
		 		';	
			}else 
			{
						 $msg .= '
				    <div class="box"  style="background:url('.$d['images'][0]['burl'].') no-repeat center top;">
				    	<div class="textcontents">'.$d['title'].'</div>
					 <span class="tag">'.$flag.$d['type'].'</span>
					<a class="boxhover" href="'.goTwoShow(array('id'=>$d['id'])).'" target="_blank">
					<div class="img"><img src="'.avatar(array('uid'=>$d['user_id'],'size'=>'m')).'" alt=""/></div>
