...
class JJUpload {
	...
	private $params; //参数
	private $defaultMaxSize = 204800; //上传文件默认最大值
	...
	private function init($name, $newName, $path) {
		...
		$file['tmp_name'] = $file['tmp_name'];
			$file['source']   = $path .'/'. $newName . '.' . 'jpg'; //路径
		$file['path']     = $path;      //目录
		$file['newName']  = $newName. '.' . 'jpg';   //文件名
		return $file;
	}
	...
	private function checkUpload($error) {
			if ($error == JJUpload::UPLOAD_ERR_INI_SIZE) { //上传是否超过ini设置
			return 'upload_err_ini_size';
		} elseif ($error == JJUpload::INPUT_MAX_FILE_SIZE) { //上传是否超过表单设置
			return 'input_max_file_size';
		} elseif ($error == JJUpload::UPLOAD_HALF) { //上传一半
			return 'upload_half';
		} elseif ($error == JJUpload::UPLOAD_ERR_NO_TMP_DIR) { //上传临时目录创建错误
	...
	public static function createFolder($path) {
		if (!is_dir($path)) {
			JJUpload::createFolder(dirname($path));
			@mkdir($path);
			@chmod($path, 0777);
