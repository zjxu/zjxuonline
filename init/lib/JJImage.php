...
class JJImage {
	 ...
	 */
	public function make_thumb($source, $newname, $width = 0, $height = 0, $isauto = false,$autowidth=0) {
		...
		//240
		//320
		if($width==0&&$height==0&&$autowidth==0)
		{
	    ...
	    {
	    	//160
	    	$height=ceil($height*(120/$width));
	    	$width=120;
			if($height>120)
			{
				//90
				    $width=ceil($width*(120/$height));
                   //120
				    $height=120;
			}
		...
		}else if($width==0&&$height==0&&$autowidth>0)
		{
				$width=$image_info['width'] ;
	    $height=$image_info['height'];
	     if($width>$autowidth)
	    {
	    	//160
	    	$height=ceil($height*($autowidth/$width));
	    	$width=$autowidth;
			if($height>$autowidth)
			{
				//90
				    $width=ceil($width*($autowidth/$height));
                   //120
				    $height=$autowidth;
			}
