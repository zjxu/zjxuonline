...
/*删除二手*/
function deltwo(id,url)
{
	$.dialog({content:'确认删除这个条信息吗？',lock:true,yesFn:function(){
...
/*提交文本框的回复*/
function sendtwomem(id,url,userid)
{
	var ipt = $('#replyInput_'+id).val();
	if(ipt == ''){tiper('内容不能为空喔');return false;}

	  $.post(url,{info:ipt,uid:userid,repuid:$('#replyTo_'+id).val(),'send':true},function(result){
		if(result ==1)
		{
			tipok('留言成功');
		}else{
...
function collect(url,id)
{
		...
		$.post(url,function(result){
			if(result ==1) {$('#mycoll_'+id).html('取消'); tipok('收藏成功'); }else if(result ==2){ tipok('您已经取消了收藏'); $('#mycoll_'+id).html('收藏');$('#myfollow_'+uid).hide(); }else{ tiper(result); }
		  });
}
/*加关注*/
function collectadd(id,url)
{
	$.post(url,{id:id},function(result){
		if(result ==1) {$('#coll_'+id).html('取消收藏'); tipok('收藏成功'); }else if(result ==2){ tipok('您已经取消了收藏'); $('#coll_'+id).html('收藏');$('#myfollow_'+uid).hide(); }else{ tiper(result); }
	  });
